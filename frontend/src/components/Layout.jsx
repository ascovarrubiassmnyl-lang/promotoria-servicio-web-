...
const IconPuntos = (p) => (<svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>);
...
const IconCandidatos = (p) => (<svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><path d="M17 11l2 2 4-4"/></svg>);
...
export default function Layout() {
  ...
  // Invitar a alguien a la promotoría: visible para cualquier rol (ver
  // InvitarUsuarioModal — 2026-09-25, pedido del usuario). No vive dentro de
  // ninguna sección con gate propio, así que el punto de entrada es global,
  // en el footer del sidebar / hoja "Más".
  ...
  const allLinks = [
    ...
    // 25 puntos y Clínica telefónica: mismas rutas para ambos roles; la API
    // resuelve el alcance (propio vs. selector por asesor para promotores).
    { to: '/puntos', label: '25 puntos', seccion: 'puntos' },
    { to: '/clinica', label: 'Clínica telefónica', seccion: 'clinica' },
  ...
  const adminLinks = [
    { to: '/candidatos', label: 'Candidatos', seccion: 'candidatos' },
  ...
  return (
    <div className="h-screen flex overflow-hidden relative app-shell">
      ...
      >
        ...
        <div className="p-2 border-t border-slate-100 dark:border-slate-700 space-y-1">
          <button
            ...
            title="Invitar a la promotoría"
          >
            <IconInvitar className="w-5 h-5 shrink-0" />
            {!colapsado && <span className="text-sm font-medium">Invitar</span>}
          </button>
          <button
            onClick={alternar}
            className={`flex items-center gap-3 rounded-lg ${colapsado ? 'h-10 w-10 justify-center mx-auto' : 'px-3 py-2'} text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/60 transition`}
        ...
        <div className="md:hidden fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Más opciones">
          ...
          <div className="absolute inset-x-0 bottom-0 max-h-[85vh] overflow-y-auto rounded-t-2xl border-t border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 pb-[calc(1rem+env(safe-area-inset-bottom))]">
            ...
            <div className="p-2">
              ...
              >
                <IconInvitar className="w-5 h-5 shrink-0" />
                <span>Invitar a la promotoría</span>
              </button>
      ...
      <InvitarUsuarioModal open={openInvitar} onClose={() => setOpenInvitar(false)} />
    </div>
  );
}
